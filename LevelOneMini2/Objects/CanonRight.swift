...
class Canon_Right: SKSpriteNode {
    ...
    init() {
        ...
        self.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: self.size.width/5, height: self.size.height/3), center: CGPoint(x: self.size.width/2, y: -120))
    ...
    func shootAnimation(){
        ...
        startAnimation(textures: canonRightTexture, speed: 0.01, name: PlayerAnimationType.shoot.rawValue, count: 0, resize: true, restore: true)
    }
